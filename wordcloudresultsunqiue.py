...
phishing_unique_words = ' '.join(set(phishing_text.split()))
not_phishing_unique_words = ' '.join(set(not_phishing_text.split()))

# Generate word clouds for phishing and not phishing phrases using unique words
phishing_wordcloud = WordCloud(width=800, height=400, background_color='white', colormap='Reds').generate(phishing_unique_words)
not_phishing_wordcloud = WordCloud(width=800, height=400, background_color='white', colormap='Greens').generate(not_phishing_unique_words)
...
bad_text = ' '.join(set(username_password_keywords))

# Generate word cloud for potential security risks using unique words
bad_wordcloud = WordCloud(width=800, height=400, background_color='white', colormap='Reds').generate(bad_text)

# Plot the word cloud
plt.figure(figsize=(8, 6))
plt.imshow(bad_wordcloud, interpolation='bilinear')
plt.title('Potential Security Risks')
plt.axis('off')
plt.show()
...
genuine_phrases = set(genuine_phrases)
not_genuine_phrases = set(bank_security_keywords) - genuine_phrases
...
genuine_reason_wordcloud = WordCloud(width=800, height=400, background_color='white', colormap='Greens').generate(' '.join(set(reasons_genuine)))

# Generate word cloud for reasons why the email might not be genuine
not_genuine_reason_wordcloud = WordCloud(width=800, height=400, background_color='white', colormap='Reds').generate(' '.join(set(reasons_not_genuine)))
